...
const Chart = ({ data }) => {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className={`${darkMode ? "bg-dark-elevated border-darkBorder" : "bg-white border-gray-200"
          } px-4 py-3 rounded-xl shadow-xl border backdrop-blur-sm`}>
          <p className={`font-medium ${darkMode ? "text-white" : "text-gray-900"}`}>
            {label}
          </p>
          <p className={`text-sm mt-1 ${darkMode ? "text-primary-blue" : "text-primary-dark"}`}>
  ...
  return (
    <div className={`${darkMode ? "bg-dark-card" : "bg-white"
      ...
      <div className="mb-6">
        ...
        <p className={`text-sm mt-1 ${darkMode ? "text-text-muted" : "text-gray-500"}`}>
        ...
        >
          ...
          >
            {chartData.map((entry, index) => {
              const gradientId = ["colorProjects", "colorStudents", "colorTasks", "colorFinished"][index];
